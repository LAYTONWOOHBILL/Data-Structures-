...
public class MyFirstHomeworkFor300 {
	...
	public static int distanceBetweenMinAndMax (double[] list) {
		...
		int min_index = 0;
		int DisBet = 0;
		double max = list[max_index];
		double min = list[min_index];
		
		for (int i=1; i<list.length; i++) {
			if (max < list[i]) {max_index = i; max=list[i];}
			if (min > list[i]) {min_index = i; min = list[i];}			
		}
		
		if (max_index > min_index) {
			DisBet =max_index -min_index;
		} else {DisBet = min_index- max_index;}
		
		return DisBet; //TODO: fix this
	}
